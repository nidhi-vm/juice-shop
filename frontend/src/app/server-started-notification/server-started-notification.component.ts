...
export class ServerStartedNotificationComponent implements OnInit {
  ...
  private restoreProgress(continueCode: string, restoreFunction: (code: string) => any, successMessageKey: string | null, errorMessageKey: string | null): void {
    restoreFunction(encodeURIComponent(continueCode)).subscribe(
      () => {
