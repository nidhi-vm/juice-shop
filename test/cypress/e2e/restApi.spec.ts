describe('/api', () => {
  describe('challenge "restfulXss"', () => {
    ...
    xit('should be possible to create a new product when logged in', () => {
      cy.task('isDocker').then(handleDockerCheck);
    });

    const handleDockerCheck = (isDocker) => {
      if (!isDocker) {
        createProduct();
      }
    };
  ...
  describe('challenge "changeProduct"', () => {
    it('should be possible to change product via PUT request without being logged in', () => {
      cy.task<number>('GetTamperingProductId').then(handleTamperingProductId);
      cy.expectChallengeSolved({ challenge: 'Product Tampering' });
    });

    const handleTamperingProductId = (tamperingProductId: number) => {
      cy.task<string>('GetOverwriteUrl').then((overwriteUrl: string) => {
        changeProduct(tamperingProductId, overwriteUrl);
      });
    };

    const changeProduct = async (tamperingProductId: number, overwriteUrl: string) => {
      await cy.window().then(async () => {
        const response = await fetch(
