describe('/#/photo-wall', () => {
  ...
  const handleSecurityQuestion = (answer: string, email: string, challenge: string) => {
  ...
  describe('challenge "geoStalkingMeta"', () => {
    it('Should be possible to find the answer to a security question in the meta-data of a photo on the photo wall', () => {
      cy.task<string>('GetFromMemories', 'geoStalkingMetaSecurityAnswer').then((answer: string) => {
        cy.task<string>('GetFromConfig', 'application.domain').then((appDomain: string) => {
          handleSecurityQuestion(answer, `john@${appDomain}`, 'Meta Geo Stalking')
        })
      })
    })
  })
  ...
  describe('challenge "geoStalkingVisual"', () => {
    it('Should be possible to determine the answer to a security question by looking closely at an image on the photo wall', () => {
      cy.task<string>('GetFromMemories', 'geoStalkingVisualSecurityAnswer').then((answer: string) => {
        cy.task<string>('GetFromConfig', 'application.domain').then((appDomain: string) => {
          handleSecurityQuestion(answer, `emma@${appDomain}`, 'Visual Geo Stalking')
        })
      })
    })
  })
