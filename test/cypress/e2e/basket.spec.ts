  ...
  describe('as admin', () => {
    ...
    describe('challenge "negativeOrder"', () => {
      it('should be possible to update a basket to a negative quantity via the Rest API', () => {
        updateBasketToNegativeQuantity();
        cy.visit('/#/order-summary');
        verifyNegativeQuantity();
      });
...
function updateBasketToNegativeQuantity() {
...
function verifyNegativeQuantity() {
...
function manipulateOtherUserBasket() {
  cy.window().then(async () => {
...
function setCouponPanelState(state: boolean) {
  cy.window().then(() => {
    window.localStorage.couponPanelExpanded = state;
  });
}
