  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (basket != null) {
        ...
        const { totalPrice, totalPoints, basketProducts, deliveryAmount, deliveryMethod } = await generatePDFContent(doc, req, basket, email, orderId, date, next)

        if (req.body.UserId) {
          await handleUserPayment(req, totalPrice, totalPoints)
        }

        await saveOrderToDatabase(req, email, orderId, totalPrice, basketProducts, totalPoints, deliveryAmount, deliveryMethod.eta.toString())
...
async function handleUserPayment(req: Request, totalPrice: number, totalPoints: number) {
...
async function generatePDFContent(doc: PDFDocument, req: Request, basket: BasketModel, email: string, orderId: string, date: string, next: NextFunction) {
  ...
  await Promise.all(basket.Products?.map(async ({ BasketItem, price, deluxePrice, name, id }) => {
    if (BasketItem != null) {
  ...
  applyDiscount(doc, req, totalPrice)
  ...
  return { totalPrice, totalPoints, basketProducts, deliveryAmount, deliveryMethod }
}
...
function applyDiscount(doc: PDFDocument, req: Request, totalPrice: number) {
...
async function saveOrderToDatabase(req: Request, email: string, orderId: string, totalPrice: number, basketProducts: Product[], totalPoints: number, deliveryAmount: number, eta: string) {
