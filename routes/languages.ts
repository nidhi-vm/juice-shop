...
export function getLanguageList() {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    fs.readFile('frontend/dist/frontend/assets/i18n/en.json', 'utf-8', (err, content) => {
      ...
      readLanguageFiles(res, next, languages, count, enContent)
    })
  }
}

function readLanguageFiles(res: Response, next: NextFunction, languages: any[], count: number, enContent: any) {
  fs.readdir('frontend/dist/frontend/assets/i18n/', (err, languageFiles) => {
    ...
    languageFiles.forEach((fileName) => {
      if (fileName.endsWith('.json')) {
        readLanguageFile(fileName, languages, count, enContent, res, next, languageFiles.length)
      } else {
        count++
      }
    })
...
function readLanguageFile(fileName: string, languages: any[], count: number, enContent: any, res: Response, next: NextFunction, totalFiles: number) {
